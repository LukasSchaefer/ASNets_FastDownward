...
static std::vector<std::pair<int, int>> get_facts_lexicographically() {
...
/* sorts operators lexicographically and returns vector of operator indeces in lexicographical ordering
   of their names */
static std::vector<int> get_operator_indeces_lexicographically() {
    ...
    for (unsigned int op_index = 0; op_index < task_proxy.get_operators().size(); op_index++) {
        auto op = task_proxy.get_operators()[op_index];
...
void ASNet::fill_input(const State& state){
    ...
    const std::vector<std::pair<int, int>> goals = task_proxy.get_goals();
    ...
    for (std::pair<int, int> fact_pair : facts_sorted) {
        ...
        if (std::find(goals.begin(), goals.end(), fact_pair) != goals.end()) {
    ...
    const std::vector<GlobalOperator> operators = task_proxy.get_operators();
    auto act_applic_values = input[2].second.matrix<float>();
    idx = 0;
    for (int operator_index : operator_indeces_sorted) {
        // check if operator is applicable
        if (operators[operator_index].is_applicable(state)) {
    ...
    if (extra_input_size) {
        auto additional_input_features = input[3].second.matrix<float>();
...
void ASNet::extract_output() {
    ...
    std::vector<OperatorID> operator_ids(operator_indeces_sorted.size());
    int idx = 0;
    for (int operator_index : operator_indeces_sorted) {
        operator_ids[idx] = operators[operator_index].get_global_operator_id();
        idx++;
    }

    last_policy_output = std::pair<std::vector<OperatorID>, std::vector<float>>(operator_ids, output_c);
}
...
static shared_ptr<neural_networks::AbstractNetwork> _parse(OptionParser &parser) {
    ...
    if (!parser.dry_run()) {       
        network = make_shared<neural_networks::ASNet>(opts);
    }
