...
# for ASNet evaluation runs:
class FastDownwardASNetRun(FastDownwardRun):
    def __init__(self, exp, algo, task):
        Run.__init__(self, exp)
        self.algo = algo
        self.task = task
        ...
        problem_name = os.path.splitext(self.task.problem_file)[0].split('/')[-1]
...
class FastDownwardASNetExperiment(FastDownwardExperiment):
    def _add_runs(self):
        for algo in self._algorithms.values():
            for task in self._get_tasks():
                self.add_run(FastDownwardASNetRun(self, algo, task))
