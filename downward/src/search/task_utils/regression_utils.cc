...
bool RegressionCondition::is_satisfied(const PartialAssignment &assignment) const {
    int current_val = assignment[data.var];
...
: RegressionConditionProxy(task, RegressionEffect(var_id, value)) { }
...
inline vector<int> get_domain_sizes(AbstractTask *task) {
    vector<int> sizes;
...
inline vector<RegressionOperator> extract_regression_operators(const AbstractTask* task) {
    verify_no_axioms();
...
: task(task->are_facts_mutex()),
possess_mutexes(g_num_mutexes() > 0),
domain_sizes(get_domain_sizes(task),
operators(extract_regression_operators(task)) { }
...
pair<bool, State> RegressionTaskProxy::convert_to_full_state(PartialAssignment& assignment,
    ...
    return make_pair(success, State(move(new_values)));
}
