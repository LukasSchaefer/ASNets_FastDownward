...
def load_build_configs(source_dir, configs=None):
    if configs is None:
        configs = {}

    for config_file in sorted(glob.glob(os.path.join(source_dir, "*build_configs.py"))):
...
CONFIGS = load_build_configs(script_dir)
